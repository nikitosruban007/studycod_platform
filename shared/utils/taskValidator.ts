...
export function validateTaskGenerationResponse(data: any): TaskGenerationSchema {
  ...
  // Validate types
  if (typeof data.title !== 'string') throw new Error('Invalid response: title must be string');
  if (typeof data.topic !== 'string') throw new Error('Invalid response: topic must be string');
  if (typeof data.difficulty !== 'number') throw new Error('Invalid response: difficulty must be number');
  if (typeof data.theoryMarkdown !== 'string') throw new Error('Invalid response: theoryMarkdown must be string');
  if (typeof data.practicalTask !== 'string') throw new Error('Invalid response: practicalTask must be string');
  if (typeof data.inputFormat !== 'string') throw new Error('Invalid response: inputFormat must be string');
  if (typeof data.outputFormat !== 'string') throw new Error('Invalid response: outputFormat must be string');
  if (typeof data.constraints !== 'string') throw new Error('Invalid response: constraints must be string');
  if (!Array.isArray(data.examples)) throw new Error('Invalid response: examples must be array');
  if (typeof data.codeTemplate !== 'string') throw new Error('Invalid response: codeTemplate must be string');
  ...
  for (let i = 0; i < data.examples.length; i++) {
    ...
    if (typeof ex.input !== 'string') throw new Error(`Invalid response: example ${i}.input must be string`);
    if (typeof ex.output !== 'string') throw new Error(`Invalid response: example ${i}.output must be string`);
